        ...
        @endif
            ...
            @for ($i = 1; $i <= 3; $i++)
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4">
                    ...
                    <livewire:special-note-title :report="$report" :index="$i" />
                    <livewire:upload-special-note-photo :report="$report" :index="$i" />
                    <livewire:special-note-description :report="$report" :index="$i" />
                </div>
            @endfor
        </div>

            <!-- 備考欄 -->
            <div id="remark" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4">
                ...
                <livewire:remark-textarea :report="$report" />
            </div>

            <div>
                <div class="flex justify-between">
                    <a href="{{ route('inspection.forth',['report' => $report->id]) }}" class="w-1/2 bg-gray-600 dark:bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-700 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 dark:focus:ring-gray-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center mr-2">
                        前へ
                    </a>
                    <a href="{{ route('confirmation',['report' => $report->id]) }}" class="w-1/2 bg-indigo-600 dark:bg-indigo-500 text-white py-2 px-4 rounded-md hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center flex items-center justify-center">
                        内容を確認
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-1 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                        </svg>
                    </a>
                </div>
