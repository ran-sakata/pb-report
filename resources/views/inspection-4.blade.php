    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md">
        <div class="flex items-center justify-between mb-6">
            <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full">
                <div class="h-2 bg-indigo-600 rounded-full" style="width: 80%;"></div>
            </div>
            <div class="flex space-x-2 text-sm text-gray-500 dark:text-gray-400 ml-4">
                <span>4 / 5</span>
            </div>
        </div>
        <h1 class="text-2xl font-bold mb-6 text-center">目視点検４</h1>
        {{-- アンカー: 架台 --}}
        <div id="panel-array" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4 scroll-mt-24">
            ...
            <livewire:panel-array-status :report="$report" />
            <p class="block text-base font-medium text-gray-700 dark:text-gray-300 mt-4 mb-4">架台画像（最大6件）</p>
            @for ($i = 1; $i <= 6; $i++)
            <div class="mb-4">
                <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">架台画像{{ $i }}</label>
                <livewire:upload-panel-array-photo :report="$report" :index="$i" :key="'panel-array-photo-'.$report->id.'-'.$i" />
            </div>
            @endfor
        </div>

        {{-- アンカー: パネル汚れ --}}
        <div id="panel-condition" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4 scroll-mt-24">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">パネル汚れ</h2>
            <p class="text-sm text-gray-700 dark:text-gray-300 mb-4">状態と画像を入力してください。</p>
            <p class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-4">状態</p>
            <livewire:panel-condition-status :report="$report" />
            <p class="block text-base font-medium text-gray-700 dark:text-gray-300 mt-4 mb-4">パネル汚れ画像（最大6件）</p>
            @for ($i = 1; $i <= 6; $i++)
            <div class="mb-4">
                <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">パネル汚れ画像{{ $i }}</label>
                <livewire:upload-panel-condition-photo :report="$report" :index="$i" :key="'panel-condition-photo-'.$report->id.'-'.$i" />
            </div>
            @endfor
        </div>
            ...
            <div>
                <div class="flex justify-between">
                    <a href="{{ route('inspection.third', ['report' => $report->id]) }}" class="w-1/2 bg-gray-600 dark:bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-700 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 dark:focus:ring-gray-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center mr-2">
                        前へ
                    </a>
                    <a href="{{ route('remark', ['report' => $report->id]) }}" class="w-1/2 bg-indigo-600 dark:bg-indigo-500 text-white py-2 px-4 rounded-md hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center">
                        次へ
                    </a>
                </div>
            </div>
