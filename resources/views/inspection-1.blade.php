...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md mt-8">
        
        <div class="flex items-center justify-between mb-6">
            <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full">
                <div class="h-2 bg-indigo-600 rounded-full" style="width: 20%;"></div>
            </div>
        ...
        {{-- ここから下は既存の各アンカーセクション --}}
        <div id="signboard-photo" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-6 scroll-mt-24">
            <a id="inspection1-anchor"></a>
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">看板写真</h2>
            <livewire:upload-signboard-photo :report="$report" />
        </div>

        {{-- 南から1~10列目 各列ごとにフォーム --}}
        <div id="row-photos" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow scroll-mt-24">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">南から1~10列目</h2>
            @for ($i = 1; $i <= 10; $i++)
                <livewire:upload-row-photo :report="$report" :row-number="$i" :key="'row-'.$i" />
            @endfor
        </div>

        {{-- 東側通路アップロード --}}
        <div id="east-path-photos" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mt-6 scroll-mt-24">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">東側通路（最大3枚）</h2>
            @for ($i = 1; $i <= 3; $i++)
                <livewire:upload-east-photo :report="$report" :row-number="$i" :key="'east-'.$i" />
            @endfor
        </div>

        {{-- 西側通路アップロード --}}
        <div id="west-path-photos" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mt-6 scroll-mt-24">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">西側通路（最大3枚）</h2>
            @for ($i = 1; $i <= 3; $i++)
                <livewire:upload-west-photo :report="$report" :row-number="$i" :key="'west-'.$i" />
            @endfor
        </div>
        ...
        <div class="flex justify-between mt-6">
            ...
            <a href="{{ route('inspection.second', ['report' => $report->id]) }}" class="w-1/2 bg-indigo-600 dark:bg-indigo-500 text-white py-2 px-4 rounded-md hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center">
                次へ
            </a>
        </div>
