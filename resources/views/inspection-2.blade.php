    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full">
                <div class="h-2 bg-indigo-600 rounded-full" style="width: 40%;"></div>
            </div>
        ...
        {{-- アンカー: 集電箱と集電箱内 --}}
        <div id="junction-box" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4 scroll-mt-24">
            <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">集電箱と集電箱内</h2>
            <!-- 集電箱 -->
            <div class="mb-4" id="junction-box-anchor">
                <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-4">集電箱</label>
                <livewire:upload-outside-junction-box-photo :report="$report" :key="'outside-junction-box-'.$report->id" />
            </div>
            <!-- 集電箱内 -->
            <div id="inside-junction-box" class="scroll-mt-24">
                <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-4">集電箱内</label>
                <livewire:upload-inside-junction-box-photo :report="$report" :key="'inside-junction-box-'.$report->id" />
            </div>
        </div>
        <!-- パワコン1～10台目 -->
        <div id="power-converters" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow mb-4 scroll-mt-24">
            ...
            @for ($i = 1; $i <= 10; $i++)
            <div class="mb-4" id="power-converter-{{ $i }}">
                <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-4">パワコン{{ $i }}台目</label>
                <livewire:upload-power-converter :report="$report" :index="$i" :key="'power-converter-'.$report->id.'-'.$i" />
            </div>
            @endfor
        </div>
        <div>
                <div class="flex justify-between">
                    <a href="{{ route('inspection.first', ['report' => $report->id]) }}" class="w-1/2 bg-gray-600 dark:bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-700 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 dark:focus:ring-gray-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800 text-center mr-2">
                        前へ
                    </a>
