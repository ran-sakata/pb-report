<div>
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($panel_condition_photo || ($report->panelConditionPhotos->firstWhere('index', $index)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        <template x-if="showInput">
            ...
            >
                ...
                <input
                    type="file"
                    wire:model="panel_condition_photo"
                    class="hidden"
                    x-ref="fileInput"
                />
            </div>
        ...
        @error('panel_condition_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($panel_condition_photo)
                    <img src="{{ $panel_condition_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->panelConditionPhotos->firstWhere('index', $index))
                ...
                <button type="button"
                    ...
                    wire:click="deletePipePuttyPhoto"
                    style="z-index:10;"
                >
