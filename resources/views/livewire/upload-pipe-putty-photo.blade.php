<div>
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($pipe_putty_photo || ($report->pipePuttyPhotos->firstWhere('index', $index)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        ...
        @error('pipe_putty_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($pipe_putty_photo)
                    <img src="{{ $pipe_putty_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->pipePuttyPhotos->firstWhere('index', $index))
