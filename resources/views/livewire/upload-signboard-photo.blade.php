<div class="mb-4">
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($signboard_photo || $report->signboard_photo_path) ? 'false' : 'true' }},
    ...
    >
        <template x-if="showInput">
            ...
            >
                ...
                <input
                    type="file"
                    wire:model.live="signboard_photo"
                    class="hidden"
                    x-ref="fileInput"
                />
            </div>
        ...
        @error('signboard_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($signboard_photo)
                    <img src="{{ $signboard_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($report->signboard_thumbnail_path)
                    <img src="{{ asset('storage/' . $report->signboard_thumbnail_path) . '?' . ($report->updated_at?->timestamp ?? now()->timestamp) }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($report->signboard_photo_path)
                    <img src="{{ asset('storage/' . $report->signboard_photo_path) . '?' . ($report->updated_at?->timestamp ?? now()->timestamp) }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @endif
                <button type="button"
                    ...
                    wire:click="deleteSignboardPhoto"
                    style="z-index:10;"
                >
