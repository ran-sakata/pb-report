<div class="mt-4">
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($west_photo || ($report->westPathPhotos->get($rowNumber - 1)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">西側通路 {{ $rowNumber }}</label>
        ...
        @error('west_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($west_photo)
                    <img src="{{ $west_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->westPathPhotos->get($rowNumber - 1))
