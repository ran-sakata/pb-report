<div class="mb-4">
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($outside_junction_box_photo || $report->junction_box_photo) ? 'false' : 'true' }},
    ...
    >
        <template x-if="showInput">
            ...
            >
                ...
                <input
                    type="file"
                    wire:model="outside_junction_box_photo"
                    class="hidden"
                    x-ref="fileInput"
                />
            </div>
        ...
        @error('outside_junction_box_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($outside_junction_box_photo)
                    <img src="{{ $outside_junction_box_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($report->junction_box_thumbnail_path)
                    <img src="{{ asset('storage/' . $report->junction_box_thumbnail_path) . '?' . ($report->updated_at?->timestamp ?? now()->timestamp) }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($report->junction_box_photo)
                    <img src="{{ asset('storage/' . $report->junction_box_photo) . '?' . ($report->updated_at?->timestamp ?? now()->timestamp) }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @endif
                <button type="button"
                    ...
                    wire:click="deleteOutsideJunctionBoxPhoto"
                    style="z-index:10;"
                >
