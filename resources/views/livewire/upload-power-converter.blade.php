<div>
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($power_converter || ($report->powerConverters->firstWhere('index', $index)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        ...
        @error('power_converter') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($power_converter)
                    <img src="{{ $power_converter->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->powerConverters->firstWhere('index', $index))
                ...
                <button type="button"
                    ...
                    wire:click="deletePowerConverter"
                    style="z-index:10;"
                >
