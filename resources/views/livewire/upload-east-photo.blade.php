<div class="mt-4">
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($east_photo || ($report->eastPathPhotos->get($rowNumber - 1)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">東側通路 {{ $rowNumber }}</label>
        ...
        @error('east_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($east_photo)
                    <img src="{{ $east_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->eastPathPhotos->get($rowNumber - 1))
