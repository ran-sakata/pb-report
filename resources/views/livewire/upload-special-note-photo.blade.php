<div class="mb-4">
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($photo || ($report->specialNotes->firstWhere('index', $index)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        <label for="special_note_photo_{{ $index }}" class="block text-base font-medium text-gray-700 dark:text-gray-300">写真</label>
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($photo)
                    <img src="{{ $photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @else
                    @php
                        $note = $report->specialNotes->firstWhere('index', $index);
                        $previewUrl = $note && $note->thumbnail_path
                            ? asset('storage/' . $note->thumbnail_path)
                            : ($note && $note->photo_path ? asset('storage/' . $note->photo_path) : null);
                    @endphp
                    @if ($previewUrl)
                        <img src="{{ $previewUrl }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                    @endif
                @endif
                <button type="button"
    ...
    @if (session('message'))
        <div class="text-green-600 dark:text-green-400 text-sm mt-2">{{ session('message') }}</div>
    @endif
</div>
