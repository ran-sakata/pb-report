<div>
    <form wire:submit.prevent="save" enctype="multipart/form-data"
        x-data="{
            progress: 0,
            isDragging: false,
            showInput: {{ ($panel_array_photo || ($report->panelArrayPhotos->firstWhere('index', $index)?->photo_path)) ? 'false' : 'true' }},
    ...
    >
        ...
        @error('panel_array_photo') <span class="text-red-500">{{ $message }}</span> @enderror
        ...
        <template x-if="!showInput">
            <div class="relative flex flex-col items-center">
                @if ($panel_array_photo)
                    <img src="{{ $panel_array_photo->temporaryUrl() }}" class="h-32 object-contain rounded-2xl border border-gray-300 dark:border-gray-600 bg-white" />
                @elseif ($photo = $report->panelArrayPhotos->firstWhere('index', $index))
                ...
                <button type="button"
                    ...
                    wire:click="deletePanelArrayPhoto"
                    style="z-index:10;"
                >
